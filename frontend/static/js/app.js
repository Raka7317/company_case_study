...
async function loadDocuments() {
  ...
  try {
    ...
    tbody.innerHTML = data.documents.map(d => `
      <tr>
        <td><a class="row-link" onclick="location.href='/documents/${encodeURIComponent(d.document_name)}/view'">${d.document_name}</a></td>
    ...
    tbody.innerHTML = `<tr><td colspan="4">Failed to load documents: ${e}</td></tr>`;
  }
}
